...
class ProjectWeeklyReport(models.Model):
    ...
    def _onchange_segment(self):
        ...
        if self.project_id and self.project_start:
            start_date = datetime.strptime(self.project_start, "%Y-%m-%d").date()
            if self.weekly_segment == 'w1':
                ...
                end_date = False
            data_set = []
            project_end = datetime.strptime(self.project_end, "%Y-%m-%d").date()
            for task in self.project_id.tasks:
                    for history in task.task_progress_ids:
                        data = history.read()[0]
                        if data.get('create_date'):
                            created_date = datetime.strptime(data.get('create_date'), "%Y-%m-%d %H:%M:%S").date()
                            print start_date,created_date,project_end
